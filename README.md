...
```bash
torne-se-reserva-estoque-app$ sam build --use-container
```
...
```bash
torne-se-reserva-estoque-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
torne-se-reserva-estoque-app$ sam local start-api
torne-se-reserva-estoque-app$ curl http://localhost:3000/
```
...
```bash
torne-se-reserva-estoque-app$ sam logs -n HelloWorldFunction --stack-name "torne-se-reserva-estoque-app" --tail
```
...
```bash
torne-se-reserva-estoque-app$ pip install -r tests/requirements.txt --user
# unit test
torne-se-reserva-estoque-app$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
torne-se-reserva-estoque-app$ AWS_SAM_STACK_NAME="torne-se-reserva-estoque-app" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "torne-se-reserva-estoque-app"
```
